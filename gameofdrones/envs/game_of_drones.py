...
from .utils_game_of_drones.PID import PID
from .utils_game_of_drones.states_drone import States, Actions, ControllerDisturbance
from .utils_game_of_drones.utils_drone import Vector, Position
from .utils_game_of_drones.world_settings import *


class GameOfDrone(gym.Env):
    """Class implementing the drone's environment and its dynamics."""
    ...
    def reset(self):
        ...
        self.target_position = np.random.choice(np.linspace(0.3 * W, 0.7 * W, num=5)), np.random.choice(
            np.linspace(0.7 * H, 0.9 * H, num=5))
        self.state_starting_position.x = np.random.choice(np.linspace(0.3 * W, 0.7 * W, num=5))
        ...
        self.state_space = States(ground_position=self.state_starting_position.normalise(),
                                  drone_position=self.state_starting_position.normalise(),
                                  drone_body_position=Position(self.drone.position.x,
                                                               self.drone.position.y).normalise(),
    ...
    def step(self, action_value):
        ...
        drone_body_position = Position(self.drone.position.x, self.helipads_outer[0].position.y)
        ...
        self.state_space.update_state(left_foot_pos=Position(position.x, position.y).set_drone_offset(),
                                      drone_body_pos=drone_body_position.set_drone_body_offset(),
...
if __name__ == "__main__":
    ...
    environment = gym.make(env_name).unwrapped
    # Reset environment
    environment.reset()
